...
public class Field {
	...
	Field(int width, int height, int mines, boolean cheatMode) {
		this.width = width;
		this.height = height;
	...
	synchronized void paint(Graphics g, Dimension size) {
	...
	void FlagBlock(Point point, Dimension dimension) {
		synchronized (blocks) {
			FlagBlockDirect(pointToCoord(point, dimension));
		}
	}
	
	void FlagBlockDirect(Point point) {
		synchronized (blocks) {
	...
	void changeBlock(Point point, Dimension dimension) {
		synchronized (blocks) {
			changeBlockDirect(pointToCoord(point, dimension));
		}
	}
	
	void changeBlockDirect(Point point) {
		synchronized (blocks) {
			if (!created) {
				created = true;
				plantMines(point);
