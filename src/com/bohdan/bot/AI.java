...
import com.bohdan.player.BlockData;
import com.bohdan.player.Click;
import com.bohdan.player.FieldView;
...
public class AI {
	...
	private LinkedList<Click> findProbabilities(LinkedList<Rule> rules) {
		...
		for (Point pt : pts) {
			...
			for (int j = 0; j < rules.size(); j++) {
				if (rules.get(j).contains(pt)) {
					b[j] = true;
				}
			}
			matrixColumns.add(new MatrixColumn(pt, b));
		}
	...
	private LinkedList<LinkedList<BasicRule>> getRowList(LinkedList<BasicRule> r) {
		LinkedList<BasicRule> allRows = new LinkedList<>(r);
	...
	private Click getBestClick(double[] percent, LinkedList<LinkedList<Point>> pts) {
		...
		LinkedList<Point> bestPointList = new LinkedList<>(pts.get(0));
	...
	private double[] getPercent(LinkedList<LinkedList<Point>> points, LinkedList<int[]> list) {
		...
		for (int[] aList : list) {
			BigInteger span = getSpan(points, aList);

			wholeSpan = wholeSpan.add(span);
			for (int i = 0; i < aList.length; i++) {
				bigPercent[i] = bigPercent[i].add(BigInteger.valueOf(aList[i]).multiply(span));
			}
		}
	...
	private LinkedList<int[]> getList(LinkedList<LinkedList<Point>> points, LinkedList<BasicRule> rows) {
		...
		LinkedList<BasicRule> basicRules = new LinkedList<>(rows);
	...
	private LinkedList<Integer> getToBeDone(LinkedList<LinkedList<Point>> points, LinkedList<BasicRule> usesRows) {
		...
		toBeDone.sort(Comparator.comparingInt(o -> points.get(o).size()));
		return toBeDone;
	}
	...
	private boolean isFirstClick(BlockData[][] blockData) {
		for (BlockData[] aBlockData : blockData) {
			for (BlockData anABlockData : aBlockData) {
				if (!anABlockData.isUnknown()) {
					return false;
				}
			}
		}
